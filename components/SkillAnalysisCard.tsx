...
const SkillAnalysisCard: React.FC<Props> = ({ data }) => {
  const chartData = [
    { name: 'Matched', value: data.matchedSkills.length, color: '#22c55e' }, // green-500
    { name: 'Missing', value: data.missingSkills.length, color: '#e2e8f0' }, // slate-200
  ];

  // Prevent empty chart
  if (data.matchedSkills.length === 0 && data.missingSkills.length === 0) {
    chartData[1].value = 1;
  }
